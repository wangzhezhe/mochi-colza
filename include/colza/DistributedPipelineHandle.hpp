 ...
 */
typedef std::function<uint64_t(const std::string&, uint64_t, uint64_t)>
    HashFunction;
...
class DistributedPipelineHandle {
 ...
 public:
  ...
  // the comm might be updated for the dynamic case
  void updateComm(ClientCommunicator* updatedComm);
   ...
   */
  void stage(const std::string& dataset_name, uint64_t iteration,
             uint64_t block_id, const std::vector<size_t>& dimensions,
             const std::vector<int64_t>& offsets, const Type& type,
             const thallium::bulk& data, const std::string& origin_addr = "",
             int32_t* result = nullptr, AsyncRequest* req = nullptr) const;
  ...
  void stage(const std::string& dataset_name, uint64_t iteration,
             uint64_t block_id, const std::vector<size_t>& dimensions,
             const std::vector<int64_t>& offsets, const Type& type,
             const void* data, int32_t* result = nullptr,
   ...
   */
  void execute(uint64_t iteration, int32_t* result = nullptr,
               bool autoCleanup = false, AsyncRequest* req = nullptr) const;
   ...
   * @param req Asynchronous request.
   */
  void cleanup(uint64_t iteration, int32_t* result = nullptr,
               AsyncRequest* req = nullptr) const;
