...
int communicator::send(const void* data, size_t size, int dest, int tag) {
    ...
    tl::provider_handle dest_pr = m_controller->member_id_to_provider_handle(m_members[dest]);
    ...
    tl::bulk local_bulk = m_controller->get_engine().expose(segment, tl::bulk_mode::read_only);
    // send the RPC
    int ret = m_controller->m_p2p_transfer_rpc.on(dest_pr)(m_comm_id, local_bulk, size, (int32_t)m_rank, (int32_t)tag);
    return ret;
}
...
int communicator::recv(void* data, size_t size, int src, int tag) {
    ...
    segment[0].first = const_cast<void*>(data);
    segment[0].second = size;
    tl::bulk local_bulk = m_controller->get_engine().expose(segment, tl::bulk_mode::write_only);
    // lock the pending request map to see if there is a matching request 
    {
        ...
        m_pending_p2p_requests_cv.wait(lock,
                [this, sig] () {
...
int communicator::on_p2p_transfer(const tl::endpoint& ep, tl::bulk& remote_bulk, size_t size, int32_t source, int32_t tag) {
    p2p_request req;
