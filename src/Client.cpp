...
PipelineHandle Client::makePipelineHandle(const std::string& address,
  ...
  if (result.success()) {
    auto pipeline_impl = std::make_shared<PipelineHandleImpl>(
        self, std::move(ph), pipeline_name);
...
DistributedPipelineHandle Client::makeDistributedPipelineHandle(
    ClientCommunicator* comm, const std::string& ssg_group_file,
    uint16_t provider_id, const std::string& pipeline_name, bool check) const {
  ...
  if (comm->rank() == 0) {
    ...
    if (ret != SSG_SUCCESS) {
      throw Exception(ErrorCode::SSG_ERROR, "Could not get group size"s);
    }
    ...
    for (int i = 0; i < group_size; i++) {
      ...
      if (ret != SSG_SUCCESS) {
        spdlog::info(
            ...
            "(ssg_get_group_member_addr returned {}",
            ret);
        ...
        for (int k = 0; k < 3; k++) {
          ...
          if (ret != SSG_SUCCESS) {
            if (k == 2) {
              throw Exception(ErrorCode::SSG_ERROR,
                              "Could not get member address second time "
      ...
      strcpy(packed_addresses.data() + i * 256,
             static_cast<std::string>(addr).c_str());
      try {
        auto pipeline =
            makePipelineHandle(addr, provider_id, pipeline_name, check);
    ...
    if (group_size != -1) {
      comm->bcast(packed_addresses.data(), packed_addresses.size(), 0);
    }
    spdlog::trace("PipelineHandle debug rank 0 group_size {} gid {} ",
                  group_size, gid);
    ...
    spdlog::trace("PipelineHandle debug nonzero rank group_size {}",
                  group_size);
    ...
    for (int i = 0; i < group_size; i++) {
      ...
      auto pipeline =
          makePipelineHandle(addr, provider_id, pipeline_name, false);
      pipelines.push_back(std::move(pipeline));
    }
  ...
  auto impl = std::make_shared<DistributedPipelineHandleImpl>(
      comm, pipeline_name, self, gid, ssg_group_file, provider_id,
      std::move(pipelines));
